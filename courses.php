...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_mapel'])) {
    ...
    if (ctype_digit($id) && $nama) {
        $stmt = $pdo->prepare("
            UPDATE subjects
            SET name = ?, class_id = ?
            WHERE id = ?
        ");
        $stmt->execute([$nama, $class_id, $id]);
    }
...
$pengajarList = [];
try {
    $stmtPengajar = $pdo->query("
        SELECT id, nama
        FROM pengajar
        ORDER BY nama
    ");
    $pengajarList = $stmtPengajar->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    $pengajarList = [];
}
...
<body>
  <div class="cover" role="application" aria-label="Admin cover dashboard">
    ...
    <div class="sidebar" role="navigation" aria-label="Sidebar">
      ...
      <div class="navlink active" onclick="location.href='courses.php'"><i class="bi bi-journal-bookmark"></i>Courses</div>
...
<tr>
  ...
  data-id="<?= e($r['id']) ?>"
  data-name="<?= e($r['nama_mapel']) ?>"
  data-class="<?= e($r['class_id']) ?>">
...
<div class="modal fade" id="modalTambah" tabindex="-1">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <form method="post">
        <input type="hidden" name="tambah_mapel" value="1">
  ...
  <input name="nama_mapel" class="form-control mb-2"
         placeholder="Nama Mapel" required>

  <input name="jenjang" class="form-control mb-2"
         placeholder="Jenjang (SD / SMP / SMA)" required>
...
<div class="modal fade" id="modalEdit" tabindex="-1">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      ...
      <form method="post">
        <input type="hidden" name="edit_mapel" value="1">
        ...
        <div class="modal-body">
          <input name="nama_mapel" id="edit_nama" class="form-control mb-2" required>
          <input name="jenjang" id="edit_jenjang" class="form-control mb-2" required>

          <select name="id_pengajar" id="edit_pengajar" class="form-control mb-2" required>
            <option value="">-- Pilih Pengajar --</option>
            <?php foreach ($pengajarList as $p): ?>
              <option value="<?= e($p['id']) ?>"><?= e($p['nama']) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  modalEdit.addEventListener('show.bs.modal', function (event) {
    ...
    // Kelas (class_id)
    document.getElementById('edit_jenjang').value = btn.dataset.class;
  });
