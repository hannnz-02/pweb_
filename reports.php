...
// total kelas
$totalKelas = $pdo->query("SELECT COUNT(*) FROM classes")->fetchColumn();

// total siswa = semua user
$totalSiswa = $pdo->query("SELECT COUNT(*) FROM users")->fetchColumn();

// total pengajar (sementara belum ada role)
// total pengajar
$totalPengajar = $pdo->query("SELECT COUNT(*) FROM mentor")->fetchColumn();
...
<body>
  <div class="cover" role="application" aria-label="Admin cover dashboard">
    ...
    <div class="sidebar" role="navigation" aria-label="Sidebar">
      ...
      <div class="navlink active" onclick="location.href='reports.php'"><i class="bi bi-graph-up"></i>Reports</div>
    ...
    <div class="main">
      ...
      <div class="hero">
  <div>
    <h2 style="margin:0; font-family:'Poppins',sans-serif">Reports</h2>
    <div style="opacity:0.9">Ringkasan data sistem</div>
  </div>
</div>
    ...
    <div class="row g-3">
      ...
      <div class="col-md-4">
        <div class="panel text-center">
          <h5>Total Kelas</h5>
          <h1 class="text-success"><?= $totalKelas ?></h1>
        </div>
      </div>
      ...
      <div class="col-md-4">
        <div class="panel text-center">
          <h5>Total Siswa</h5>
          <h1 class="text-primary"><?= $totalSiswa ?></h1>
        </div>
      </div>

      <div class="col-md-4">
        <div class="panel text-center">
          <h5>Total Pengajar</h5>
          <h1 class="text-warning"><?= $totalPengajar ?></h1>
        </div>
      </div>
